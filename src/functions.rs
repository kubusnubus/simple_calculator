...
pub fn fact(n: f64) -> f64 {
    for k in 0..=21 {
        ...
        if k == 21 {
            println!("ERROR: factorial: argument is not one of 0..=21 (range limited for the sake of accuracy)");
            std::process::exit(0)
        }
    }
    if n == 0.0 {
...
pub fn sin(a: f64) -> f64 {
    ...
    for n in 0..=10 {
        if n % 2 == 0 {
            res += (x.powf(2.0 * (n as f64) + 1.0)) / fact(2.0 * (n as f64) + 1.0)
        } else {
            res -= (x.powf(2.0 * (n as f64) + 1.0)) / fact(2.0 * (n as f64) + 1.0)
        }
    }
...
pub fn tan(x: f64) -> f64 {
    if cos(x) == 0.0 {
        println!("ERROR: tan: argument not in the domain");
        std::process::exit(0)
    }
    sin(x) / cos(x)
}
...
pub fn arctan(x: f64) -> f64 {
    arcsin(x / (1.0 + x.powf(2.0)).powf(0.5))
}
...
pub fn ln(x: f64) -> f64 {
    //println!("{}", x);
    if x <= 0.0 {
        println!("ERROR: ln: argument not in the domain");
        std::process::exit(0)
    }
    let mut i: f64 = -1.0;
    /*integer approximation of i:*/
    loop {
        if (x - 1.0).is_sign_negative() {
            break;
        }
        if exp(i) < x && x <= exp(i + 1.0) {
            break;
        }
        i += 1.0
    }
...
pub fn log(a: f64, b: f64) -> f64 {
    if b <= 0.0 {
        println!("ERROR: log: second argument not in the domain");
        std::process::exit(0)
    }
    if a == 1.0 || a <= 0.0 {
        println!("ERROR: log: first argument not in the domain");
        std::process::exit(0)
    }
    ln(b) / ln(a)
}
...
pub fn productlog(branch: f64, x: f64) -> f64 {
    if x < -exp(-1.0) {
        println!("ERROR: W: second argument not in the domain");
        std::process::exit(0)
    }
    if branch != -1.0 && branch != 0.0 {
        println!("ERROR: W: first argument not one of -1, 0");
        std::process::exit(0)
    }
    if branch == -1.0 && x >= 0.0 {
        println!("ERROR: W: for second argument >= 0, only the 0 branch is available");
        std::process::exit(0)
    }
    let mut i: f64 = 0.0;
    if branch == -1.0 {
...
pub fn sqrt(x: f64) -> f64 {
    if x < 0.0 {
        println!("ERROR: sqrt: argument not in the domain");
        std::process::exit(0)
    }
    x.powf(0.5)
}
...
pub fn modulo(a: f64, b: f64) -> f64 {
    if b == 0.0 {
        println!("ERROR: mod: the second argument cannot be zero");
        std::process::exit(0)
    }
    a - b * floor(a / b)
}
...
pub fn sum(start_tmp: f64, stop_tmp: f64, exprstart: usize, exprstop: usize, list_tmp: Vec<Vec<String>>) -> f64 {
    ...
    if floor(stop_tmp) != stop_tmp {
        println!("ERROR: sum: the second argument must be an integer");
        std::process::exit(0)
    }
...
pub fn prod(start_tmp: f64, stop_tmp: f64, exprstart: usize, exprstop: usize, list_tmp: Vec<Vec<String>>) -> f64 {
    if floor(start_tmp) != start_tmp {
        println!("ERROR: prod: the first argument must be an integer");
        std::process::exit(0)
    }
    if floor(stop_tmp) != stop_tmp {
        println!("ERROR: prod: the second argument must be an integer");
        std::process::exit(0)
    }
    let start: usize = start_tmp.floor() as usize;
    let stop: usize = stop_tmp.floor() as usize;
    ...
    'outer: loop {
        for ch in exprstart..=exprstop {
            if list[ch][1] == "x" {
                ...
                for i in 0..ch {
                    list_tmp_tmp.push(list[i].clone())
                }
...
pub fn der(a: f64, exprstart: usize, exprstop: usize, list_tmp: Vec<Vec<String>>) -> f64 {
    ...
    'outer: loop {
        for ch in exprstart..=exprstop1 {
            if list1[ch][1] == "x" {
                let mut list_tmp_tmp = vec![];
                for i in 0..ch {
                    list_tmp_tmp.push(list1[i].clone())
                }
                list_tmp_tmp.push(vec!["".to_string(), "(".to_string()]);
                list_tmp_tmp.push(vec!["".to_string(), "(".to_string()]);
                list_tmp_tmp.push(vec!["".to_string(), n.to_string()]);
    ...
    for ch in exprstart..=exprstop {
        if list2[ch][1] == "x" {
            let mut list_tmp_tmp = vec![];
            for i in 0..ch {
                list_tmp_tmp.push(list2[i].clone())
            }
            list_tmp_tmp.push(vec!["".to_string(), a.to_string()]);
            for i in ch + 1..list2.len() {
                list_tmp_tmp.push(list2[i].clone())
            }
            list2 = list_tmp_tmp;
        }
